...
public class DataGenerator {
    ...
    public static final Instant BEGINNING = Instant.parse("2022-10-25T12:00:00.00Z");
    ...
    public long carId() {
        ...
        return 2022000000 + rnd.nextInt(NUMBER_OF_VEHICLES);
    }
    ...
    public float startLat() {
        return aFloat((float) (GeoLocationUtils.LAT_SOUTH - 0.1), (float) (GeoLocationUtils.LAT_NORTH + 0.1F));
    }

    /** Deterministically generates and returns the startLon for this ride. */
    public float startLon() {
        return aFloat((float) (GeoLocationUtils.LON_WEST - 0.1), (float) (GeoLocationUtils.LON_EAST + 0.1F));
    }

    /** Deterministically generates and returns the endLat for this ride. */
    public float endLat() {
        return bFloat((float) (GeoLocationUtils.LAT_SOUTH - 0.1), (float) (GeoLocationUtils.LAT_NORTH + 0.1F));
    }

    /** Deterministically generates and returns the endLon for this ride. */
    public float endLon() {
        return bFloat((float) (GeoLocationUtils.LON_WEST - 0.1), (float) (GeoLocationUtils.LON_EAST + 0.1F));
    }
    ...
    public float totalFare() {
        return (float) (3.0 + (1.0 * rideDurationMinutes()) + tolls());
    }
